...
def read_tiles(locations, queue_out, filename0, filename1, opt):
    if not locations:
        raise ValueError("Worker has no locations to process")
    slide0, slide1 = WSIReader(filename0, opt), WSIReader(filename1, opt)
    read_level_patch_size = int(opt.patch_size * opt.mpp/(slide0.mpp_x*slide0.level_downsamples[slide0.read_level]))
    ...
    for x, y in locations:
        ...
        assert tile0.shape[0] == read_level_patch_size and tile0.shape[1] == read_level_patch_size
        if read_level_patch_size != opt.patch_size:
        ...
        if self.opt.workers > 0:  # parallelise
            ...
            for i in range(self.opt.workers):
                ...
                if path0 is not None and path1 is not None:
                    ...
                    process = mp.Process(target=read_tiles, args=(split, queue_out, self.file_name, self.opt))
                process.start()
    ...
    def apply(self, function, output_dtype, save_path):
        ...
        if self.shift_and_merge:
            ...
            with TemporaryFile() as merged_temporary_file:
                ...
                for x, y, input_tile, shifted_input_tile in offset_tiles:
