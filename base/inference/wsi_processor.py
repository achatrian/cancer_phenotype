...
def read_tile(locations, queue_out, filename, opt):
    if not locations:
        ...
        return
    slide = WSIReader(filename, opt, opt.set_mpp)
...
def read_tiles(locations, queue_out, filename0, filename1, opt):
    ...
    if not locations:
        warnings.warn("Worker has no locations to process")
        return
    slide0, slide1 = WSIReader(filename0, opt, opt.set_mpp), WSIReader(filename1, opt, opt.set_mpp)
    if opt.force_base_level_read:
...
class WSIProcessor:
    ...
    def __init__(self, file_name, opt, shift_and_merge=True, normalize_output=False, filter_location=None, set_mpp=None,
                 tissue_mask=None, tissue_mask_info=None, force_base_level_read=True):
        r"""
        ...
        self.slide = WSIReader(self.file_name, opt, set_mpp)  # not actually used for reading regions in extract_tiles
        ...
        if tissue_mask is None:
            ...
            if tile_read_errors:  # if any instance of OpenSlideError is raised, slide properties become unreadable
                ...
                self.slide = WSIReader(self.file_name, opt, set_mpp)
    ...
    def extract_tiles(self, locations, path0=None, path1=None):
        ...
        if self.opt.workers > 0:  # parallelize
            ...
            slide0 = WSIReader(self.file_name, self.opt, getattr(self.opt, 'set_mpp', None)) if path0 is None \
                else WSIReader(path0, self.opt, getattr(self.opt, 'set_mpp', None))
            slide1 = None if path1 is None else WSIReader(path0, self.opt, getattr(self.opt, 'set_mpp', None))
            ...
            for x, y in locations:
                try:
                    tile0 = np.array(slide0.read_region((x, y), 0, (read_level_patch_size,) * 2))[..., :3]
                    tile0 = resize(tile0, (self.opt.patch_size,)*2, preserve_range=True).astype(np.uint8)
                    if slide1 is not None:
                        tile1 = np.array(slide1.read_region((x, y), 0, (read_level_patch_size,) * 2))[..., :3]
                    ...
                    slide0 = self.slide if path0 is None else \
                        WSIReader(path0, self.opt, getattr(self.opt, 'set_mpp', None))
                    slide1 = None if path1 is None else WSIReader(path0, self.opt, getattr(self.opt, 'set_mpp', None))
                    if self.opt.force_base_level_read:
                        slide0.read_level = 0
                        if slide1:
    ...
    def apply(self, function, output_dtype, save_path):
        ...
        try:
            prob_map = WSIReader(file_name=str(prob_map_path), opt=self.opt, set_mpp=self.opt.set_mpp)
        ...
        if self.shift_and_merge:
            ...
            try:
                shifted_prob_map = WSIReader(file_name=str(shifted_prob_map_path), opt=self.opt, set_mpp=self.opt.set_mpp)
                if self.opt.overwrite:
