...
def lab_transfer(img0, seg0, img1, seg1):
    lab0 = color.rgb2lab(img0)
    lab1 = color.rgb2lab(img1)

    lab0_ = lab0.copy()

    linear_space = np.linspace(0, 100, 255)

    xp = np.percentile(lab0[seg0 > 0, 0], linear_space)
    fp = np.percentile(lab1[seg1 > 0, 0], linear_space)
    lab0_[:, 0] = np.interp(lab0[:, 0], xp, fp)

    xp = np.percentile(lab0[seg0 > 0, 1], linear_space)
    fp = np.percentile(lab1[seg1 > 0, 1], linear_space)
    lab0_[:, 1] = np.interp(lab0[:, 1], xp, fp)

    xp = np.percentile(lab0[seg0 > 0, 2], linear_space)
    fp = np.percentile(lab1[seg1 > 0, 2], linear_space)
    lab0_[:, 2] = np.interp(lab0[:, 2], xp, fp)

    img0 = color.lab2rgb(lab0_)
    img0 = (255.0 * img0).astype(np.uint8)
