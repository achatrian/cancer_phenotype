...
def process_image(images, input_path, model):
    ...
    model.test()
    output_logits = model.get_current_visuals()['output_map']  # FIXME make this work for batch_size > 1 # 'logits' as they are not bounded and we want to take softmax on them
...
if __name__ == '__main__':
    ...
    for image_path in image_paths:
        ...
        try:
            ...
            processor.apply(process_image_with_model, np.uint8, Path(opt.data_dir)/'data'/opt.mask_dirname)
        ...
        rescale_factor = mask_slide.level_downsamples[processor.slide.read_level]  # to original images
        ...
        for i in range(nw+1):
            for j in range(nh+1):
                ...
                mask_tile = np.array(mask_slide.read_region((x+i*512, y+j*512),
                                                            processor.slide.read_level, (x_len, y_len)))[..., :3]
                mask[j*512:(j+1)*512, i*512:(i+1)*512, :] = mask_tile
        converter = MaskConverter()
        print("Extracting contours from mask ...")
        contours, labels, boxes = converter.mask_to_contour(np.array(mask), x, y, rescale_factor=None)  # don't rescale map inside
        # rescale contours
