...
class ApplyOptions(BaseOptions):
    def __init__(self):
        super().__init__()
        parser = self.parser
        parser.add_argument('--deployer_name', type=str, default='segment', choices=['segment'], help="Format of images onto which model is applied")
        parser.add_argument('--results_dir', type=str, default='/well/rittscher/users/achatrian/Results/applied', help='saves results here.')
        ...
        parser.set_defaults(dataset_name='tcga')
        self.is_train = False
        self.parser = parser
