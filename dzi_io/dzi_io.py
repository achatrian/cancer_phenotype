...
class DZI_IO(object):
    ...
    def __init__(self, src, target=None, clean_target=False):
        ...
        if os.path.exists(os.path.join(self.src_dir, 'properties.json')):
            self.properties = json.load(open(os.path.join(self.src_dir, 'properties.json'), 'r'))
            if self.target is not None:
                if not os.path.exists(self.target_dir):
                    os.makedirs(self.target_dir)
                json.dump(self.properties, open(os.path.join(self.target_dir, 'properties.json'), 'w'))
            if 'mpp' in self.properties:
                self.mpp = self.properties['mpp']
            elif 'openslide.mpp-x' in self.properties:
                self.mpp = self.properties['openslide.mpp-x']
            elif 'openslide.mpp-y' in self.properties:
                self.mpp = self.properties['openslide.mpp-y']
        ...
        for level in range(self.level_count):
            ...
            for i in range(n_col+1):
                for j in range(n_row+1):
                    ...
                    rotated = rotate_bound(tile, angle)
