...
from repo_old.ck5.ck5_dataset import ERGGlandDataset
from repo_old.ck5.ck5_utils import eval_classification, DiceLoss
from segment.utils import on_cluster, get_time_stamp, check_mkdir, str2bool, AverageMeter, get_flags
...
def main(FLAGS):
    ...
    criterion1 = CrossEntropyLoss(size_average=True, weight=torch.tensor([1.0, 3.0]).float())
    criterion2 = DiceLoss(weight=torch.tensor([1.0, 5.0]).float())
    if cuda.is_available():
    ...
    train_dataset = ERGGlandDataset(str(FLAGS.data_file), "train", tile_size=FLAGS.image_size, augment=True)
    val_dataset = ERGGlandDataset(str(FLAGS.data_file), "val", tile_size=FLAGS.image_size, augment=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data_file', type=str, default="/gpfs0/users/rittscher/achatrian/cancer_phenotype/ck5/glands_ERGlabel.csv")
    ...
    if on_cluster():
        ckpt_path = "/well/rittscher/users/achatrian/cancer_phenotype/logs/" + "erg_" + timestamp + "/ckpt"
    else:
