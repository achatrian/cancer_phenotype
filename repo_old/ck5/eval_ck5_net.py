...
from repo_old.ck5.ck5_dataset import CK5Dataset, CK5GlandDataset
from repo_old.ck5.ck5_utils import eval_classification, DiceLoss
from segment.utils import on_cluster, get_time_stamp, str2bool, AverageMeter
...
def main(FLAGS):
    ...
    if FLAGS.full_glands:
        test_dataset = CK5GlandDataset(str(FLAGS.data_dir), "test", tile_size=FLAGS.image_size, augment=False)
    else:
        test_dataset = CK5Dataset(str(FLAGS.data_dir), "test", augment=False)
...
if __name__ == '__main__':
    ...
    if on_cluster():
        ckpt_path = "/well/rittscher/users/achatrian/cancer_phenotype/logs/" + "ck5_" + timestamp + "/ckpt"
    else:
