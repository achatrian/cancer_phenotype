...
from prostate_dataset import SegDataset
...
from phenotyping.gland_dataset import GlandPatchDataset, AugDataset
from segment.prostate_dataset import AugDataset as SegAugDataset
...
def main(FLAGS):
    ...
    if FLAGS.full_glands:
        if FLAGS.augment_dir:
            ...
            train_dataset = AugDataset(FLAGS.data_dir, FLAGS.augment_dir, "train", tile_size=FLAGS.image_size,
                                       augment=FLAGS.augment, generated_only=FLAGS.generated_only)
        else:
            train_dataset = GlandPatchDataset(FLAGS.data_dir, "train", tile_size=FLAGS.image_size,
        ...
        if FLAGS.augment_dir:
            print("Loading augmented dataset")
            train_dataset = SegAugDataset(FLAGS.data_dir, FLAGS.augment_dir, "train", tile_size=FLAGS.image_size,
                                          augment=FLAGS.augment, generated_only=FLAGS.generated_only)
        else:
            train_dataset = SegDataset(FLAGS.data_dir, "train", tile_size=FLAGS.image_size, augment=FLAGS.augment)
        val_dataset = SegDataset(FLAGS.data_dir, "val", tile_size=FLAGS.image_size, augment=0)
...
if __name__ == '__main__':
    ...
    if on_cluster():
        ckpt_path = "/well/win/users/achatrian/cancer_phenotype/logs/" + timestamp + "/ckpt"
    else:
