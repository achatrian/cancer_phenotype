...
from phenotyping.feature_utils import make_thumbnail, make_thumbnail_fullcut, create_sprite_image, to_tensor, augment_glands, \
    FeatureExtractSummarise
from phenotyping.gland_dataset import GlandPatchDataset, GlandDataset
...
def main(FLAGS):
    ...
    N = FLAGS.augment_num
    extract_layers = ['input_block', 'enc1', 'enc2', 'enc3', 'enc4', 'enc5', 'enc6', 'center']
    features_net = FeatureExtractSummarise(net, extract_layers, N)
    if cuda.is_available():
    ...
    if FLAGS.full_glands:
        datasets = [GlandPatchDataset(FLAGS.data_dir, mode, tile_size=512, return_cls=True) for mode in ['train', 'test']]
    else:
        datasets = [GlandDataset(FLAGS.data_dir, mode, tile_size=512, augment=False) for mode in ['train', 'test']]
    loaders = [DataLoader(ds, batch_size=FLAGS.batch_size, drop_last=True, shuffle=False, num_workers=FLAGS.workers) for ds in datasets]
    ...
    for loader, mode in zip(loaders, ['train', 'test']):
        ...
        for i, data in enumerate(tqdm(loader)):
            ...
            for j, (img, gt, label) in enumerate(zip(imgs[0::(N + 1), ...], gts, label_batch)):
                ...
                gt = gt.squeeze()
                if FLAGS.full_glands:
                    tn = make_thumbnail(img, gt, th_size, label=label)
                else:
