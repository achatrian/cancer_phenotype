...
class ResidualAttentionModel_448input(nn.Module):
    ...
    def forward(self, x):
        ...
        self.attention_maps.append(self.attention_module0.attention_map[0].max(0).squeeze())
        # print(out.data)
        ...
        self.attention_maps.append(self.attention_module2_2.attention_map[0].max(0).squeeze())
        out = self.residual_block3(out)
        # print(out.data)
        out = self.attention_module3(out)
        out = self.attention_module3_2(out)
        out = self.attention_module3_3(out)
        self.attention_maps.append(self.attention_module3_3.attention_map[0].max(0).squeeze())
...
class ResidualAttentionModel_56(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.residual_block1(out)
        out = self.attention_module1(out)
        self.attention_maps.append(self.attention_module1.attention_map[0].max(0).squeeze())
        out = self.residual_block2(out)
        out = self.attention_module2(out)
        self.attention_maps.append(self.attention_module2.attention_map[0].max(0).squeeze())
        out = self.residual_block3(out)
        # print(out.data)
        out = self.attention_module3(out)
        self.attention_maps.append(self.attention_module3.attention_map[0].max(0).squeeze())
        out = self.residual_block4(out)
        out = self.residual_block5(out)
        out = self.residual_block6(out)
        out = self.mpool2(out)
        out = out.view(out.size(0), -1)
        out = self.fc(out)
        assert len(self.attention_maps) == 3
        return out
